...
impl CargoBuild {
    ...
    async fn build(self: Arc<Self>) -> Result<CargoBuildOutput> {
        ...
        match build_result {
            ...
            Err(err) => return Err(anyhow!("error calling cargo: {}", err)),
        }
        ...
        let artifacts_out = Command::new("cargo")
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|err| anyhow!("error getting cargo artifact info: {}", err))?
            .output()
            .await
            .map_err(|err| anyhow!("error calling cargo: {}", err))?;
        ...
        let wasm_bytes = async_std::fs::read(&wasm).await?;
